...
name: instalar
description: >
  Instala o NERION OS no negócio do usuário. Entrevista sobre empresa, tom de voz,
  foco atual e identidade visual, e preenche `_memoria/empresa.md`, `_memoria/preferencias.md`,
  `_memoria/estrategia.md`, `identidade/design-guide.md` e adapta o `NERION-OS.md` conforme o perfil.
  Use quando o usuário acabou de clonar o repositório e quer instalar o sistema, ou quando
  pedir explicitamente "rodar /instalar", "instalar o NERION OS", "primeiro setup".
---

# /instalar — Instalação inicial do NERION OS
...
Conferir o nome da pasta atual (`basename "$(pwd)"`). Se for `nerion-os`, `NERION OS`, `NERION OS-main`, `nerionos-main` ou variação genérica:

> "Notei que a pasta atual ainda tem nome genérico ('<nome-atual>'). O ideal é a pasta ter o nome do seu negócio, não 'NERION OS'. Quando terminarmos o setup, te lembro de renomear (é rápido — fechar VS Code, renomear a pasta no Finder/Explorer, abrir de novo). Bora seguir?"
...
A resposta determina qual template de `NERION-OS.md` aplicar (ver `templates/perfis/`).
...
### `NERION-OS.md`
Pegar o template correspondente ao perfil escolhido na Fase 1 (`templates/perfis/codex-md-<perfil>.md`), adaptar com o nome do negócio e estrutura de pastas mencionada nas respostas, e sobrescrever o `NERION-OS.md` da raiz.
...
✓ NERION-OS.md adaptado pro perfil [perfil]
```
...
> "Pronto. O NERION OS já te conhece.
>
